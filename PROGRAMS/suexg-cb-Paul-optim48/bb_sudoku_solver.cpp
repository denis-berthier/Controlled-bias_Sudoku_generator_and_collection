...
// forward procedure declarations
       int  DecodePuzzleString (int ret_puz, int* buffer);
      ...
      { SCnt++;
        ...
        { if (!No_Sol)
          { if (!PuzSolCnt && ret_puzzle)
              for (i = 0; i < 81; i++) buffer[i] = B2V[SolGrid[i]];
            if (PuzSolCnt && (ret_puzzle == 2))
              for (i = 0; i < 81; i++)
                if (buffer[i] != (B2V[SolGrid[i]])) buffer[i] = '0';
            PuzSolCnt++;
...
int DecodePuzzleString (int ret_puz, int *buffer)
{
  ...
  for (i = 0; i < 81; i++)
    {
      if (buffer[i] > 0)
        PushSingle (i, V2B[buffer[i]]);
    }
  Givens = SingleCnt;
  return (USE_GUESSES | USE_LOCK_CAND);
}
...
{ static int LastPos = 0;
  ...
  { // Find next spot to check
    ...
    for (j = 0; j < NSet[G[PIdx].Grid[testcell]]; j++)
    {
      ...
      bb_Solver(1, new_method, 0, PIdx, NULL);
...
{
  if (bb_Solver(2, 0x01, 0, 0, buf) != 1) return 0;  // Invalid (No Solution, or mulitples)
  if (bb_Solver(1, 0x3E, 0, 0, buf) == 0) return 5;  // Insane (none exist, that I know of)
